\# 🛒 Retail Performance Analytics Dashboard



\## 📌 Overview

An end-to-end data analytics project built using:



\- Python (Data Cleaning \& Feature Engineering)

\- PostgreSQL (Database Modeling)

\- Power BI (Dashboard \& Business Intelligence)



This project analyzes retail sales data (2011–2014) to generate business insights across revenue, profitability, region performance, and customer intelligence.



---



\## 🧩 Tech Stack



\- Python (Pandas, NumPy)

\- PostgreSQL

\- Power BI

\- DAX

\- SQL



---



\## 🔄 Project Workflow



\### 1️⃣ Data Cleaning (Python)



\- Date conversion

\- Delivery days calculation

\- Profit margin calculation

\- Null handling

\- Feature engineering (Year, Month)



📂 File: `python/superstore\_cleaning.py`



---



\### 2️⃣ Database Design (PostgreSQL)



\- Schema creation

\- Data type optimization

\- Bulk import using COPY



📂 File: `sql/superstore\_schema.sql`



---



\### 3️⃣ Power BI Dashboard (4 Pages)



\#### 📊 Executive Summary

\- Total Revenue

\- Total Profit

\- Profit Margin

\- Orders Count



\#### 📈 Sales Analysis

\- Monthly Revenue Trend

\- YoY Growth

\- Top 10 Products

\- Category Performance



\#### 🌍 Regional Analysis

\- Geo Bubble Map

\- Region Matrix

\- Delivery Performance



\#### 👤 Customer Intelligence

\- Top 20 Customers

\- Repeat Customers

\- Retention %

\- Segmentation



---



\## 📊 Key Business Insights



\- Revenue grew steadily from 2011 to 2014

\- Technology category leads profitability

\- Certain regions show high revenue but low margins

\- Repeat customers contribute significantly to total revenue



---



\## 📷 Dashboard Preview



!\[Executive Summary](images/executive\_summary.png)



---



\## 🚀 What This Project Demonstrates



✔ End-to-End Data Pipeline  

✔ Data Cleaning \& ETL  

✔ SQL Schema Design  

✔ Time Intelligence (SAMEPERIODLASTYEAR)  

✔ Geo Analytics  

✔ Customer Retention Modeling  

✔ Professional BI Dashboard Design  



---



\## 👨‍💻 Author



Harish Gouda  

MCA | Data Analytics Enthusiast  

